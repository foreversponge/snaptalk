...
export default class PicColor extends Component {
  ...
  render() {
    ...
    const option = this.options.map((option, index) => {
      const borderBottom =
        index === this.options.length - 1 ? null : (
      ...
      return (
        <TouchableOpacity
          ...
          <Text
            style={
              this.state.selectedItem === option
                ? [styles.text, {color: option}]
                : styles.text
            }>
    ...
    return (
      <TouchableOpacity
        ...
        <View
          style={[
            styles.modal,
            {width: this.state.width - 80, height: this.state.height - 80},
          ]}>
          <ScrollView style={styles.optionView}>{option}</ScrollView>
          <TouchableHighlight
            onPress={() => this.closeModal(false, this.state.selectedItem)}
