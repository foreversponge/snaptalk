...
describe('RegisterScreen Test', () => {
  test('Empty string username', async () => {
    ...
    const usernameError = await AuthenticationController.shared
      .createUser(usernameEmpty)
      .catch(error => {
  ...
  test('Invalid string username (more than 20 character)', async () => {
    ...
    const usernameError = await AuthenticationController.shared
      .createUser(usernameLength26)
      .catch(error => {
        return error.message;
      });
    expect(usernameError).toBe(
      'Username must only contain 20 letters and/or numbers.',
    );
  });
  ...
  test('Invalid string username (invalid characters)', async () => {
    ...
    const usernameError = await AuthenticationController.shared
      .createUser(usernameInvalidChar)
      .catch(error => {
        return error.message;
      });
    expect(usernameError).toBe(
